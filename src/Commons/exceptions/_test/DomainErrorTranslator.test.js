...
describe('DomainErrorTranslator', () => {
  it('should translate error correctly', () => {
    ...
    // thread domain error translator
    ...
    expect(DomainErrorTranslator.translate(new Error('THREAD.TITLE_LIMIT_CHAR')))
        .toStrictEqual(new InvariantError('tidak dapat membuat thread baru karena karakter title melebihi batas limit: max_character[100]'));
    expect(DomainErrorTranslator.translate(new Error('THREAD.OWNER_LIMIT_CHAR')))
        .toStrictEqual(new InvariantError('tidak dapat membuat thread baru karena karakter owner melebihi batas limit: max_character[50]'));
    expect(DomainErrorTranslator.translate(new Error('THREAD.OWNER_CONTAIN_RESTRICTED_CHARACTER')))
        .toStrictEqual(new InvariantError('tidak dapat membuat thread baru karena owner mengandung karakter terlarang'));

    // comment domain error translator
